...
const PaginateChildRowsGrid = () => {
    ...
    const serverSideDatasource: IServerSideDatasource = useMemo(() => ({
        getRows: (params) => {
            const dataRequest = fetch('http://localhost:8080/docs/pagination/paginate-child-rows/getRows', {
                method: 'POST',
                headers: {
            ...
            if (pagination && refetchRowCount) {
                const countRequest = fetch('http://localhost:8080/docs/pagination/paginate-child-rows/countRows', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
